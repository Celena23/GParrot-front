  ...
  <div class="card">
    ...
    <div class="card">
      <DataTable ref="dt" :value="commentaires" v-model:selection="selectedCommentaires" dataKey="id"
                 ...
                 currentPageReportTemplate="Résultats {first} à {last} sur {totalRecords} commentaires">
        <template #header>
          ...
          <div class="flex flex-wrap gap-2 align-items-center justify-content-between">
            <Button label="Ajouter un commentaire" icon="pi pi-plus" severity="success" class="mr-2"
        ...
        <Column field="valid" header="Visible" sortable style="min-width:12rem">
          <template #body="slotProps">
            <InputSwitch v-model="slotProps.data.valid" @change="validComment(slotProps.data)"/>
          </template>
...
const error = ref()
axios
    .get("http://localhost:8080/commentaire", {headers: {"Content-Type": "application/json"}})
    .then((response: any) => {
